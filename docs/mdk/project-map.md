...
- Optional cross-version config model and declaration code. No Spawner does not
  currently use this source set.
...
- Optional version-specific config support. No Spawner does not currently use
  this source set.
...
- Does not apply `fabric-api-conventions` or `fabric-config-conventions`
  unless a future change actually needs those dependencies.

`<mc>-forge`

- Legacy Forge entrypoint, resources, mixin config, and templates.
- Uses `legacyforge-mod-conventions`.
- Applies `legacyforge-config-conventions` only when config code is included.

`<mc>-neo`

- NeoForge entrypoint, resources, mixin config, and templates.
- Uses `neoforge-mod-conventions`.
- Applies `neoforge-config-conventions` only when config code is included.
