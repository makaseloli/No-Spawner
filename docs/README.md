# No Spawner MDK guide

No Spawner uses `Meatwo310/custom-mdk` as a multi-version, multi-loader Gradle
build. It keeps the custom-mdk build logic, generated loader metadata,
versioned artifact names, Java toolchain management, runtime mod staging, and
...
LLM agents and automation should also read [MDK Agent Notes](mdk/README.md)
...
- `src/config`: optional config-related source set from custom-mdk. No Spawner
  does not currently apply config conventions or ship config dependencies.
- `src/configClient`: optional client-only config screen helpers. No Spawner
  does not currently use this source set.
...
Before opening or importing the project in IntelliJ IDEA or Gradle, check
`settings.gradle.kts`: each included project adds Gradle configuration and IDE
import load time.

1. Edit `settings.gradle.kts` only when adding or removing supported targets.
2. Edit `gradle.properties` for shared mod metadata and Fabric entry points.
3. Edit `version.txt` for the release version used by all platform jars.
4. Put loader metadata shape in each platform's `src/main/templates`.

## Generated Metadata

Fabric `fabric.mod.json` files are generated from shared values in
`gradle.properties`, `version.txt`, and the Fabric convention plugin. The
convention provides common fields such as the mod id, version, authors, contact
URLs, environment, entry points, Java requirement, Minecraft requirement, and
Fabric Loader requirement. No Spawner does not apply Fabric API or config
conventions, so Fabric API, Mod Menu, Forge Config API Port, and Forge Config
Screens are not production or runtime dependencies.
...
./gradlew :26.1-neo:runServer
./gradlew :26.2-fabric:runServer
./gradlew :26.2-neo:runServer
```

## Dependencies

No Spawner has no extra mod dependencies beyond the selected loader and
Minecraft toolchain. Use `gradle/libs.versions.toml` for shared dependency
aliases if a future change needs one, and keep target versions in the affected
subproject's `gradle.properties`.
...
No Spawner currently has no config files and does not apply any config
convention plugin. The custom-mdk config system remains available for future
changes, but adding it would also require adding the matching loader bindings
and dependencies described below.
...
When a version-specific common project such as `26.2-common` needs extra entries, append them before a platform binds the declarations:
...
When a platform such as `26.2-fabric` or `26.2-neo` needs its own entries, append them in the entry point before calling `PlatformConfigRegistrar`:
